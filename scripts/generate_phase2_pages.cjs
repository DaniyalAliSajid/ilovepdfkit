...
// JSX string attribute helper
// JSX prop="value" cannot contain backslash-escaped quotes.
// When a string contains ", use {"..."}  JSX expression syntax instead.
function jsxStrAttr(str) {
  if (str.includes('"')) {
    // Use template literal expression in JSX: prop={`...`}
    // Escape backticks and ${} in the string for template literal safety
...
function generatePageFile(page) {
  ...
  if (page.features) page.features.forEach(f => { if (f.icon) iconsUsed.add(f.icon); });
  if (page.toolSpotlights) page.toolSpotlights.forEach(t => { if (t.icon) iconsUsed.add(t.icon); });
  const iconsImportStr = Array.from(iconsUsed).join(', ');

  const fnName = page.slug.split('-').map(s => s.charAt(0).toUpperCase() + s.slice(1)).join('') + 'Page';

  const featuresJsx = page.features
    .map(f => `        { icon: ${f.icon}, title: ${JSON.stringify(f.title)}, description: ${JSON.stringify(f.description)} }`)
    .join(',\r\n');
  ...
  const lines = [
    ...
    `  return (`,
    `    <SEOLandingPage`,
    `      slug=${jsxStrAttr(page.slug)}`,
    `      keyword=${jsxStrAttr(page.keyword)}`,
    `      pageType=${jsxStrAttr(page.pageType)}`,
    `      angle=${jsxStrAttr(page.angle)}`,
    `      h1=${jsxStrAttr(page.h1)}`,
    `      heroDescription=${jsxStrAttr(page.heroDescription)}`,
    `      intro=${jsxStrAttr(page.intro)}`,
    `      steps={[`,
    ...page.steps.map((s, i) => `        { title: ${JSON.stringify(s.title)}, description: ${JSON.stringify(s.description)} }${i < page.steps.length - 1 ? ',' : ''}`),
    `      ]}`,
    `      features={[`,
    featuresJsx,
    `      ]}`,
  ...
  if (page.useCases) {
    lines.push(`      useCases={[`);
    page.useCases.forEach((u, i) => lines.push(`        { title: ${JSON.stringify(u.title)}, description: ${JSON.stringify(u.description)} }${i < page.useCases.length - 1 ? ',' : ''}`));
    lines.push(`      ]}`);
  }

  if (page.toolSpotlights) {
    lines.push(`      toolSpotlights={[`);
    page.toolSpotlights.forEach((t, i) => lines.push(`        { name: ${JSON.stringify(t.name)}, href: ${JSON.stringify(t.href)}, description: ${JSON.stringify(t.description)}, icon: ${t.icon} }${i < page.toolSpotlights.length - 1 ? ',' : ''}`));
    lines.push(`      ]}`);
  }

  if (page.comparisonTable) {
    lines.push(`      comparisonTable={${JSON.stringify(page.comparisonTable, null, 2)}}`);
  }

  lines.push(`      midCTA={{ text: ${JSON.stringify(page.midCTA.text)}, href: ${JSON.stringify(page.midCTA.href)} }}`);
  lines.push(`      bottomCTA={{ headline: ${JSON.stringify(page.bottomCTA.headline)}, text: ${JSON.stringify(page.bottomCTA.text)}, href: ${JSON.stringify(page.bottomCTA.href)} }}`);
  lines.push(`      faqs={[`);
  page.faqs.forEach((faq, i) => lines.push(`        { question: ${JSON.stringify(faq.question)}, answer: ${JSON.stringify(faq.answer)} }${i < page.faqs.length - 1 ? ',' : ''}`));
  lines.push(`      ]}`);
  lines.push(`      faqTitle=${JSON.stringify(page.faqTitle)}`);
  lines.push(`      faqSubtitle=${JSON.stringify(page.faqSubtitle)}`);
  lines.push(`      breadcrumbs={[`);
  page.breadcrumbs.forEach((b, i) => lines.push(`        { name: ${JSON.stringify(b.name)}, href: ${JSON.stringify(b.href)} }${i < page.breadcrumbs.length - 1 ? ',' : ''}`));
  lines.push(`      ]}`);
  lines.push(`      themeColor=${JSON.stringify(page.themeColor)}`);
  lines.push(`    />`);
  lines.push(`  );`);
  lines.push(`}`);
  lines.push(``);
