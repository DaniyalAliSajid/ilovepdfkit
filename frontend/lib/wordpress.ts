const WP_BASE_URL = process.env.WP_BASE_URL || 'https://cms.ilovepdfkit.com';
...
export async function getAllPosts(): Promise<WPPost[]> {
    try {
        const response = await fetch(`${WP_API_URL}/posts?_embed&per_page=100&status=publish`, {
            next: { tags: ['wordpress'], revalidate: 3600 } // Revalidate every hour OR when webhook hits
        });
...
export async function getPostBySlug(slug: string): Promise<WPPost | null> {
    try {
        const response = await fetch(`${WP_API_URL}/posts?slug=${slug}&_embed`, {
            next: { tags: ['wordpress', `wordpress-${slug}`], revalidate: 3600 }
        });
